...
namespace TensorRtSharp
{
    ...
    public class Nvinfer
    {
        ...
        public void init(string engine_filename, int num_ionode, string input_node_name, string output_node_name)
        {
            ptr = NativeMethods.nvinfer_init(engine_filename, num_ionode, input_node_name, output_node_name);
        }

        public int[] read_model_info(string input_node_name, string output_node_name)
        {
            int[] model_info = new int[5];
            NativeMethods.read_model_info(ptr, input_node_name, output_node_name, ref model_info[0]);
            return model_info;
        }
        /// <summary>
        /// 创建gpu显存缓存
        ...
        public void load_image_data(string node_name, byte[] image_data, int image_Cols, int image_Rows, BNFlag BN_means)
        {
            ptr = NativeMethods.load_image_data(ptr, node_name, ref image_data[0], image_Cols, image_Rows, (int)BN_means);
        }
        /// <summary>
        /// 模型推理
        /// </summary>
        ...
        public int[] read_infer_result(string node_name_wchar, MMSFlag NMS_means)
        {
            int[] result = new int[6000];
            NativeMethods.read_infer_result(ptr, node_name_wchar, ref result[0], (int) NMS_means);
            return result;
        }
        /// <summary>
        /// 删除模型内存地址
